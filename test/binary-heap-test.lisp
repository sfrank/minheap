(defpackage :heap-binary-test (:use :CL :binary-heap :lisp-unit))

(in-package :heap-binary-test)

(define-test test-basic
  (let ((heap (make-instance 'binary-heap:binary-heap))
...
(define-test test-meld
  (let ((heap-1 (make-instance 'binary-heap:binary-heap))
        (heap-2 (make-instance 'binary-heap:binary-heap)))
...
(define-test test-heapify
  (multiple-value-bind (heap list)
      (loop for i below 100
            for v = (random 10000)
            collect (cons v v) into alist
            collect v into vlist
            finally (return (values (alist-to-heap alist) vlist)))
    (assert-eql 100 (heap-size heap))
    (dolist (key (sort list #'<))
      (assert-true (= key (extract-min heap))))
    (assert-true (empty-p heap))))
